...
amp = AmpInterpKerrEccEq()  # Amplitue module (KerrEccEq)

teuk_modes = amp(a, p_traj, e_traj, xI_traj)

# get ylms
ylm_gen = GetYlms(include_minus_m=True) # Spin weighted spherical harmonics

ylms = ylm_gen(amp.unique_l, amp.unique_m, qS, phiS).copy()[amp.inverse_lm]

# select modes

mode_selector = ModeSelector(
    amp.l_arr_no_mask, amp.m_arr_no_mask, amp.n_arr_no_mask, force_backend="gpu"
)

kappa = 1e-5  # tolerance on mode contribution to total power

modeinds = [amp.l_arr, amp.m_arr, amp.n_arr]

(teuk_modes_in, ylms_in, ls, ms, ns) = mode_selector(
    teuk_modes, ylms, modeinds, mode_selection_threshold=kappa,
)
...
fp = "MCMC_samps_M_1e5_mu_1e3_a_0p998_p0_74p94184_e0_0p85_pro_SNR_200_eps_0p0_recov_eps_1e-3_dt_0p5_T_2_TDI2_w_background_equal_arms.h5"
breakpoint()
